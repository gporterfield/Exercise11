...
abstract class WeatherData {
    ...
    WeatherData(String dType, Double measuredValue) {
        this.dType = dType;
...
class PressureSensor implements WeatherDataSource {
    ...
    @Override
    public void update() {
        System.out.println("Pressure Sensor: getting new data.");
        updatePressure();
        for (WeatherDataListener listener: listeners) {
...
class WeatherStation implements WeatherDataListener {
    ...
    public void displayCurrentWeather() {
        ...
        for (String dType: allWeatherData.keySet()) {
            System.out.println(dType + ": " + allWeatherData.get(dType));
        }
    }
